...
curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" --data @./target.json "${SYMPHONY_API_URL}targets/registry/ankaios-target"
...
curl -s -X DELETE -H "Authorization: Bearer $TOKEN" "${SYMPHONY_API_URL}targets/registry/ankaios-target"
